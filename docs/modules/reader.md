...
- Canvas-native annotations: selection tooltip can create and edit `mark` annotations, including color + style (`Highlight` / `Underline`), persist them to SQLite, and restore/render them after repagination
...
- Mark annotation: selecting text shows a tooltip with `Phonetics`, `Explain`, `Mark/Edit/Unmark`, and `Read Aloud`. Tapping `Mark` creates the mark immediately using the current per-book default appearance. After creation, a compact style bar appears above the tooltip so the user can switch color and style (`Highlight` / `Underline`) for the focused mark without a separate confirm step. Reader remembers the last chosen mark appearance per book via `ReaderPreferences`. Reader keeps a separate persisted-mark overlay layer under the live selection layer. Annotation restore uses `blockIndex + raw offsets` first, then same-block / same-chapter text fallback with normalized `quoteText + prefix/suffix`. Creating a mark that exactly duplicates or overlaps an existing mark is rejected.
- Edit / Unmark annotation: tapping an existing mark in the reader opens a compact style bar above the tooltip directly. Color or style changes apply immediately to the focused mark, and `Unmark` removes it. If a selection overlaps multiple existing marks, Reader only offers `Unmark` and does not support batch appearance edits.
- Marks panel: the bottom toolbar annotation icon opens a side panel listing all marks for the current book. Tapping a mark performs a preview jump to that location and shows `Back to previous location`. The preview jump itself does not immediately overwrite persisted reading progress; progress updates only after the user continues reading from the previewed location.
...
- Marks panel: toolbar icon opens a side panel; tapping a list item preview-jumps to the marked text and offers a one-tap return to the previous location without immediately overwriting saved progress
