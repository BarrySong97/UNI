...
- EPUB parser opens packages in lenient mode for malformed-but-common href issues and follows manifest fallback chains when a spine item points at non-markup content
- EPUB parser preserves nested lists inside list items instead of flattening them into sibling paragraphs
- EPUB parser keeps semantic fixed-layout content such as absolutely positioned headings, paragraphs, images, figures, and figcaptions instead of discarding them as decorative out-of-flow nodes
...
- Controls overlay: bottom icon toolbar (TOC, marks list, progress, theme cycle, font settings "A")
...
- Canvas-native annotations: selection tooltip can create and edit `mark` annotations, add notes through a large keyboard-safe note sheet, persist mark + note data to SQLite, and restore/render them after repagination
- Selection tooltip actions include `Phonetics`, `Explain`, `Copy`, `Mark`, and `Read Aloud`; short word/phrase pronunciation lazily creates the active TTS voice on first playback, then uses in-memory WAV playback and a bounded LRU cache to reduce repeated tap-to-sound latency while longer selections keep the file-backed playback path
- Reader explain sheet supports long-press selection inside both structured and Markdown responses, showing inline `IPA`, `Pronounce`, and `Copy` actions for the selected word or short phrase; when local phonetics and cached AI phonetics both miss, the leading `IPA` label is replaced by an `AI` button that can fetch and cache IPA on demand
...
- On tablet widths, the Quote Card side editor keeps tall bookmark-style template tiles with square corners and renders a larger in-tile style preview so presets stay recognizable without losing the bookmark silhouette
...
lib/
  ...
  pages/reader/
    ...
    widgets/
      ...
      reader_explain_sheet.dart     # AI explain bottom sheet (word header + Markdown response + visual reference)
      reader_selection_handle.dart  # Draggable teardrop selection handle widget
  services/search/
    image_search_service.dart       # Web-scraping image search (Bing/Google/Baidu)
```
...
- `AnnotationStore`: caches per-book `AnnotationEntity` items and notifies Reader when marks are created/updated/deleted
...
- `ReadingProgressEntity(bookId, locatorJson, percent, updatedAt, prefsJson, pageCountsJson)`: locatorJson stores `{"chapterIndex": N, "pageIndex": M, "blockAnchor": B, "chapterPageCount": C, "vw": W, "vh": H}`, where `blockAnchor` is the `blockIndex` of the first block-level element on the current page (viewport-independent content anchor), `chapterPageCount` is the total pages in the current chapter at save time, and `vw/vh` record the viewport dimensions at save time. On restore, if the viewport differs, the block anchor is resolved to the correct page via `_pageIndexForBlockAnchor()`; if block resolution fails, a chapter-proportional fallback (`pageIndex/chapterPageCount * newPageCount`) is used. Old locators without these fields are fully backward compatible. `percent` stores read-based progress (`bookReadPercent`), prefsJson stores per-book ReaderPreferences as JSON, pageCountsJson stores persisted page count cache with layout parameter validation
- `ChapterPagination`: cached per chapter, invalidated on layout parameter changes
- `AnnotationEntity(bookId, kind, style, quoteText, anchorJson, color, note, createdAt, updatedAt)`: persisted in `annotations`; `style` currently supports `highlight` and `underline`, and `anchorJson` stores versioned Canvas-native block anchors instead of Readium CFI
...
- `PhoneticsCacheTable(normalized_text, us_ipa, uk_ipa, source, created_at, updated_at)`: global AI phonetics cache keyed by normalized selected text (DB v20)
- `PosService` (offline part-of-speech lookup): a read-only SQLite asset (`assets/dict/pos_dict.sqlite`, ~1.4 MB, ~88k rows) built from Princeton WordNet 3.1 via `tool/build_pos_dict.dart`. Schema: `pos_dict(word TEXT PRIMARY KEY, pos TEXT)` where `pos` is a compact code containing any of `n` (noun), `v` (verb), `a` (adjective), `r` (adverb). On first launch the asset is copied from `rootBundle` into the app's databases directory and opened read-only via `sqflite`. Lookups reuse `buildPhoneticsFallbackCandidates` so common regular inflections (`-s`, `-es`, `-ies`, `-ed`, `-ing`, `-ly`) are stripped at query time; irregular forms (`ran`, `oxen`, `children`, ...) are pre-baked into the SQLite via WordNet's `*.exc` files. WordNet does not cover function words (e.g., `the`, `of`), so those return `null` and the AI partOfSpeech still wins for them.
...
- Centered image-only page in dual mode: when a spread has only one page (no right page) and that page contains only image elements (`PageLayout.isImageOnly`), the page is centered across the full screen width instead of being left-aligned in the left half. The page indicator is also centered. This provides a better visual for full-page illustrations and cover images on tablets.
...
- External page-turn device support: listens for keyboard events from Bluetooth page turners, stylus pens, and similar HID devices. Arrow Right/Down/PageDown = next page, Arrow Left/Up/PageUp = previous page. Reuses the tap-based page turn animation. Key repeat is supported for rapid page turning.
...
- Mark annotation: selecting text shows a tooltip with `Phonetics`, `Explain`, `Mark/Edit/Unmark`, `Note`, and `Read Aloud`. Tapping `Mark` creates the mark immediately using the current per-book default appearance. Tapping `Note` opens a large note surface: on phone it is a tall bottom sheet, and on dual-page it mirrors Explain by sliding in a half-width panel from the opposite side of the selection so the selected text stays visible. The sheet owns keyboard inset handling, so the Reader root page does not shift upward when the keyboard appears. On an unmarked selection it creates the mark plus the first note, and on an existing mark it appends a new note. After creation, a compact style bar appears above the tooltip so the user can switch color and style (`Highlight` / `Underline`) for the focused mark without a separate confirm step. Reader remembers the last chosen mark appearance per book via `ReaderPreferences`. Reader keeps a separate persisted-mark overlay layer under the live selection layer. Annotation restore uses `blockIndex + raw offsets` first, then same-block / same-chapter text fallback with normalized `quoteText + prefix/suffix`. Creating a mark that exactly duplicates or overlaps an existing mark is rejected.
- Edit / Unmark annotation: tapping an existing mark still opens the original floating tooltip. Long-pressing an existing single mark opens a focused mark detail sheet. On phone it opens from the bottom; on dual-page it mirrors Explain and slides in from the opposite side of the pressed text. The focused detail reuses the same editorial mark-detail layout as the marks panel: centered `MARK DETAIL` header, large serif quote, `GO TO THE MARK`, `YOUR THOUGHT`, and `Add` / `Share` / `Delete`. When the inline note composer is open, the `Add` entry switches to `Cancel` so the user can dismiss unfinished input. If a selection overlaps multiple existing marks, Reader still does not support batch appearance edits or batch note creation.
- Marks panel: the bottom toolbar annotation icon opens a fullscreen bottom sheet with Weread-style mark cards, search, and sort, and supports swipe-down dismissal. Each card shows the quote, latest note, relative activity time, and uses the same palette family as the Shelf `Words` card. Opening a mark shows editorial `Mark Details` with chapter-only subtitle, large serif quote, `GO TO THE MARK`, thought cards, and the floating `Add` / `Share` / `Delete` action bar. Preview jump still does not immediately overwrite persisted reading progress; progress updates only after the user continues reading from the previewed location.
- AI Explain: selecting text shows a tooltip with "Explain" button. Tapping it opens a bottom sheet that calls an OpenAI-compatible LLM to explain the passage. Opening the sheet never starts TTS automatically; audio playback remains an explicit action through `Pronounce` or `Read Aloud`. Before lookup, the reader trims accidental leading/trailing punctuation from the selection so image search, context-sentence matching, cache keys, AI requests, and phonetics lookups stay stable even when the user slightly over-selects punctuation. In tablet (dual-page) mode, the sheet appears as a half-width side panel on the opposite side of the selection: text selected on the right page shows the panel on the left, and vice versa, so the selected text stays visible. For single words/phrases: shows word header + phonetics row + one context sentence (word highlighted). When the selection is a single word, the structured explain response also includes an AI-provided `partOfSpeech` label and the header renders it as a small chip above the phonetics row. To avoid waiting for the AI round-trip, the chip is filled immediately from the offline `PosService` (WordNet) when available, and is later overwritten by the AI response if it differs. The phonetics row first tries local offline lookup using an ordered fallback pipeline (`raw` -> sanitized -> lowercase -> hyphen spacing/removal -> camel split -> ordinal normalization -> common inflection fallback) and stops at the first hit. If local lookup and cached AI lookup both miss, the inline phonetics slot shows an `AI` button; tapping it queries the configured OpenAI-compatible endpoint for structured US/UK IPA, stores the result in `phonetics_cache`, and refreshes the row in place. Selections with boundary-only sentence terminators (e.g. trailing `.` or `。` that are stripped during normalization) stay in word/phrase mode so image search and phonetics remain available; only selections with internal sentence terminators (multi-sentence text) switch to the long-text preview flow. In built-in structured mode it prioritizes fast-in-context understanding with two cards (`Meaning Explain`, `Detail Explain`) plus optional `partOfSpeech` for single-word selections, and for compatibility it falls back to Markdown if structured JSON parsing fails. Structured mode also reads the source/book-language `AiLanguageConfig` and, for English books only, injects a Vocabulary Level instruction (`CET4`, `CET6`, `IELTS`, `TOEFL`, `GRE`) into the prompt. `explanationLanguage` still controls only the answer language, while `vocabularyLevel` controls the expected English-reading difficulty of the source-language explanation strategy. In custom-prompt mode, the sheet renders free-form Markdown output directly and ignores Vocabulary Level. Sentence extraction uses `renderNodePlainText()` on the original `ParagraphNode` (via `LayoutElement.sourceNode`) to get paragraph text with proper spacing, bypassing K-P / greedy layout fragmentation. `extractSelectedText()` also inserts spaces between K-P word fragments sharing the same `sourceNode`. Prompt detail levels: Brief (1-2 sentences), Balanced (short paragraph, default), Detailed (thorough but focused). Uses `ExplainAiService` backed by Genkit + OpenAI plugin. API credentials configured in Settings page via `AiSettingsService` (SharedPreferences). Each tap on Explain increments per-book `explain_count` in `book_stats` table and persists the structured response in `explain_cache`, which Shelf reuses for the `Words` preview and `More` history page. Existing cache entries remain readable after a Vocabulary Level change; only an explicit refresh regenerates the response and overwrites the same cache key.
...
- Phonetics lookup count: each tap on the "Phonetics" tooltip button increments per-book `phonetics_count` in `book_stats` table (DB v15). Phonetics lookups reuse the same boundary-punctuation trimming as Explain. Read Aloud also trims accidental boundary punctuation for selections; only multi-sentence text (with internal sentence terminators) preserves the raw text for TTS. The shared TTS engine uses Kokoro English voices for the built-in `en_US` / `en_GB` paths, lazily creates the active voice only when playback is requested, plays short text (<= 80 characters) from in-memory WAV bytes with an LRU cache, and keeps longer utterances on unique temp wav paths that are deleted on stop/completion. Reader releases TTS engines and short-audio cache on backgrounding or memory pressure. Reader does not add Piper-specific synthesis or playback hacks on top of Kokoro output. When local lookup misses, Reader does not auto-call AI; the user must explicitly tap the `AI` affordance to query and cache IPA.
...
- Controls overlay with slide animation: top bar slides down (back + more menu), bottom icon toolbar slides up (5 buttons: TOC, marks list, progress, theme, font). TOC/progress/theme/font use inline panels; marks list opens a side panel. Only one inline panel is visible at a time; tapping a different button switches directly.
...
- Preference changes keep controls overlay visible (no close on setting change). Font size, margin, line spacing, and font family changes use block-anchor-based position preservation so the user stays on the same content after re-pagination.
- Viewport change detection: `build()` monitors `MediaQuery` for viewport size, safe area, and dual-page mode changes (caused by screen rotation, iPad split-screen, or window resize). On change, `updateViewport()` snapshots the current block anchor, re-paginates, and resolves the block anchor to the correct page in the new layout.
...
- Mark annotation: selecting text → tap `Mark` to persist immediately → optional style-bar edits update color/style instantly → the saved mark still renders after reopening the book or changing ReaderPreferences
- Marks panel: toolbar icon opens a fullscreen bottom sheet; tapping a list item preview-jumps to the marked text and offers a one-tap return to the previous location without immediately overwriting saved progress
- AI Explain: selecting text → tap Explain → the reader trims accidental leading/trailing punctuation from the selection before image lookup, sentence matching, cache keys, and AI requests, while still keeping sentence selections out of word/phrase mode. The bottom sheet shows a word header + one context sentence for word/phrase selections, and text preview + AI explanation for longer sentence selections. Single-word selections additionally show an AI-provided part-of-speech chip. Default mode uses built-in structured cards (`Meaning Explain`, `Detail Explain`, optional `partOfSpeech`) and, for English books, applies the configured English Vocabulary Level from the source-language `AiLanguageConfig`. `explanationLanguage` controls output language only. Custom prompt mode (when enabled in settings) renders free-form Markdown and ignores Vocabulary Level. Unconfigured API key shows SnackBar prompt.
...
- Tapping Explain increments `explain_count` in `book_stats` for the current book and refreshable explain content is stored in `explain_cache`
