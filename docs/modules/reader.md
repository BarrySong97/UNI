...
- Edit / Unmark annotation: tapping an existing mark in the reader opens the action tooltip without re-selection and immediately expands the compact style bar for that mark. The tooltip provides `Phonetics`, `Explain`, `Note`, `Unmark`, and `Read Aloud` directly, without an extra `Edit` button. If a selection overlaps multiple existing marks, Reader only offers `Unmark` and does not support batch appearance edits or batch note creation.
- Marks panel: the bottom toolbar annotation icon opens a fullscreen bottom sheet with Weread-style mark cards, search, and sort, and supports swipe-down dismissal. Each card shows the quote, latest note, relative activity time, and `Go to location`. Opening a mark also shows its note timeline and supports `Add Note`. Preview jump still does not immediately overwrite persisted reading progress; progress updates only after the user continues reading from the previewed location.
