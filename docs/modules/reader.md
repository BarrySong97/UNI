...
- Edit / Unmark annotation: tapping an existing mark in the reader opens a compact style bar above the tooltip directly. Color or style changes apply immediately to the focused mark; `Note` appends a new note to that mark; `Unmark` removes it. If a selection overlaps multiple existing marks, Reader only offers `Unmark` and does not support batch appearance edits or batch note creation.
