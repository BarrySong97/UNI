...
## 模块目的
在 Canvas Reader 中提供持久化标注能力。当前只启用 `mark` 类型：用户在 Reader 里选中文本后可通过 Tooltip 创建、编辑、删除 mark，并为 mark 选择颜色与样式（`Highlight` / `Underline`）。后续支持基于同一套 annotation 数据结构扩展 note / bookmark 等类型。
...
2. 点击 Tooltip 的 `Mark` 会立即创建 mark；点击已有 mark 或 `Edit` 会打开紧凑的 mark style bar。
...
5. 新建 mark 时直接使用当前默认颜色与样式；style bar 中可切换 `Highlight`（底色）或 `Underline`（下划线），并立即更新当前 mark。
6. `AnnotationStore.createMark()` / `updateAnnotationAppearance()` 创建或更新 `AnnotationEntity` 并写入 `annotations` 表。
...
- `annotations` 表
  ...
  - `created_at`
  - `updated_at`
- `AnnotationStore.state.items / isLoading / selectedColor / selectedStyle`
...
- 用户直接点击正文里已有的 mark 时，弹出已展开的 mark editor tooltip，可修改颜色/样式，或点击 `Unmark` 删除当前 mark。
- 当一次选区命中多个已有 marks 时，只允许 `Unmark`，不支持批量修改颜色/样式。
...
## 验收标准
- Tooltip 中存在 `Mark` / `Edit` / `Unmark` 动作。
- 用户点击 `Mark` 后会立即看到持久化标记；在样式栏切换颜色或样式时，当前页会立即更新。
